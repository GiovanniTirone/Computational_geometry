                            ...
                            F extends FaceDCEL<D,V,P,VTX,E,F>>
{
    ...
    public PolygonDCEL (List<P> points,Class<VTX> class_VTX,Class<E> class_E, Class<F> class_F) throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        //set prev and next for remaining elements
        {
            ...
            edgeList.get(1).setPrev(edgeList.get(3));
            ...
            edgeList.get(edgeList.size()-1).setPrev(edgeList.get(1));
        }
    ...
    public void getEdgesWithSameFaceAndGivenOrigins(
    ...
    public void printVertices () {
        vtxList.forEach(vtx -> System.out.println(vtx.getPoint().getDetails()));
    }

    public void printEdges () {edgeList.forEach(edge -> System.out.println(edge.origin.getPoint().getDetails() + " -> " +edge.destination().getPoint().getDetails()));}

    public void printFacesOfEdges () {
            System.out.println("Outer edges: ");
            for (int i=0; i<edgeList.size() ; i+=2){
                System.out.println(edgeList.get(i).origin.getPoint().getDetails() + " -> " +edgeList.get(i).destination().getPoint().getDetails());
            }
    ...
    public void printFaces () {
        for(F face : faceList){
            System.out.println("FACE with vertices : " + face.getOuterPoints());
            E outer = face.outer;
            System.out.println(outer.origin.getPoint().getDetails() + " -> " + outer.destination().getPoint().getDetails());
            E next = outer.next;
            while(next != outer){
                System.out.println(next.origin.getPoint().getDetails() + " -> " +next.destination().getPoint().getDetails());
                next = next.next;
            }
        }
    }
